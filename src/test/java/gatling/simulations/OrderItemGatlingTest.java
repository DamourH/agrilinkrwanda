...
public class OrderItemGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all orderItems").get("/api/order-items").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new orderItem")
                        .post("/api/order-items")
                        .headers(headersHttpAuthenticated)
                        .body(StringBody("{" + "\"quantity\": 0" + ", \"priceAtOrder\": 0" + "}"))
                        .asJson()
                        .check(status().is(201))
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created orderItem").get("${new_orderItem_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created orderItem").delete("${new_orderItem_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );
