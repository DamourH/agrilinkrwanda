...
public class FarmerProduceGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all farmerProduces").get("/api/farmer-produces").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new farmerProduce")
                        .post("/api/farmer-produces")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_farmerProduce_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created farmerProduce").get("${new_farmerProduce_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created farmerProduce").delete("${new_farmerProduce_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the FarmerProduce entity").exec(scn);
