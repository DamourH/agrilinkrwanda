...
public class PurchaseOrderGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all purchaseOrders").get("/api/purchase-orders").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new purchaseOrder")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_purchaseOrder_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created purchaseOrder").get("${new_purchaseOrder_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created purchaseOrder").delete("${new_purchaseOrder_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the PurchaseOrder entity").exec(scn);
