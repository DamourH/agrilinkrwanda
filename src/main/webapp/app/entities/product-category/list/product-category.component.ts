...
export class ProductCategoryComponent implements OnInit {
  ...
  search(query: string): void {
    ...
    if (query && predicate && ProductCategoryComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(predicate)) {
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      const { predicate } = this.sortState();
      if (predicate && ProductCategoryComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(predicate)) {
        this.sortState.set({});
      }
    }
  }
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.productCategories.set(this.refineData(dataFromBody));
  }

  protected refineData(data: IProductCategory[]): IProductCategory[] {
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.productCategoryService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    }
