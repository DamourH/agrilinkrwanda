...
type RestOf<T extends IDelivery | NewDelivery> = Omit<T, 'pickupDate' | 'deliveryDate'> & {
...
export class DeliveryService {
  ...
  protected resourceSearchUrl = this.applicationConfigService.getEndpointFor('api/deliveries/_search');
  ...
  search(req: Search): Observable<EntityArrayResponseType> {
    const options = createRequestOption(req);
    return this.http.get<RestDelivery[]>(this.resourceSearchUrl, { params: options, observe: 'response' }).pipe(
      map(res => this.convertResponseArrayFromServer(res)),

      catchError(() => scheduled([new HttpResponse<IDelivery[]>()], asapScheduler)),
    );
  }
  ...
  protected convertDateFromClient<T extends IDelivery | NewDelivery | PartialUpdateDelivery>(delivery: T): RestOf<T> {
    return {
      ...delivery,
      pickupDate: delivery.pickupDate?.toJSON() ?? null,
  ...
  protected convertDateFromServer(restDelivery: RestDelivery): IDelivery {
    return {
      ...restDelivery,
      pickupDate: restDelivery.pickupDate ? dayjs(restDelivery.pickupDate) : undefined,
